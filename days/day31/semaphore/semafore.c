 ...
 * parm 2:(1):No of semaphores i.e if u have more than one critical regions that has to be synchronized u can give a value other than one here
 
 * parm 3:flags that gives perms and to create a semaphore with those perms */
	
//sem_union.val=1;
//if(semctl(sem_id,0,SETVAL,sem_union)<0) //semaphore initialization
//		printf("error\n");
for(i=0;i<=10;i++)
{
	get_semaphore();
