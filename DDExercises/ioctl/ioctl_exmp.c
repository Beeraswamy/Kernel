...
static int cparam1,cparam2,cparam3,cparam4;
...
printk("<1>" "%x %x %x %x\n",MYDEV_GET_CP1,MYDEV_SET_CP2,MYDEV_SET_CP3,MYDEV_GET_CP3);
  switch(cmd)
  {
	...
	ret = get_user(cparam3, (unsigned char*)arg);
	printk("<1>"  "cparam3 %c arg %x",cparam3,arg);    
	break;

    case MYDEV_GET_CP3: 
      //ret = __put_user(cparam3, (unsigned char*) arg);
ret = put_user(cparam3, (unsigned char*)arg);
printk("<1>"  "cparam3 %c *arg %x",cparam3,arg); 
      break;
    case MYDEV_SET_GET_CP4:
	ret = __get_user(cparam4, (unsigned char*) arg);
        ret = __put_user(cparam4, (unsigned char*) arg);
	printk("<1>"  "cparam4 %u arg %x",cparam4,arg);
	break;
    default:
      return -1;
  }
